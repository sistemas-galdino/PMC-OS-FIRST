// Guia "como encontrar o Guardião" — conteúdo didático reutilizado na Visão Geral
// do Guardião E na Fase 1 do Método (quando o cliente ainda não escolheu).
// Blocos: 1) conceito · 2) 4 passos · (slot dinâmico) · 3) 5 pilares · 4) perfil ideal.
// `onIr(tab)` decide a navegação (setSearchParams no Guardião; navigate no Método).
...
export const PASSOS = [
  { numero: 1, titulo: "Convide", desc: "Gere um link único por pessoa — colaboradores internos ou candidatos de fora. Cada um recebe o mesmo teste.", cta: "Gerar convite", tab: "convites", icon: Send },
  { numero: 2, titulo: "Eles respondem", desc: "O assessment avalia os 5 pilares do Guardião + perfil DISC. Leva cerca de 15 minutos, direto pelo link.", cta: null, tab: null, icon: FileText },
  { numero: 3, titulo: "Compare", desc: "Cada pessoa recebe uma nota geral e uma nota por pilar. Quem atinge 70% ou mais tem perfil apto.", cta: "Ver candidatos", tab: "candidatos", icon: BarChart3 },
  { numero: 4, titulo: "Conduza o funil", desc: "Com os aprovados, siga as etapas: envio de case, entrevista, negociação e contratação do seu Guardião.", cta: "Abrir funil", tab: "candidatos", icon: Users },
]

const PILARES = [
  { title: "Adoção à tecnologia", desc: "Curiosidade, abertura e prática real com ferramentas de IA no dia a dia.", exemplo: "Já usa ChatGPT, Claude ou outras IAs por conta própria para resolver problemas do trabalho — sem ninguém mandar.", icon: Bot },
  { title: "Visão de processo e resultado", desc: "Enxerga o todo, mapeia processos e foca em ganho prático para o negócio.", exemplo: "Consegue descrever um processo da empresa do início ao fim e apontar onde se perde tempo ou dinheiro.", icon: Target },
  { title: "Comunicação e liderança", desc: "Engaja pessoas, traduz a IA para diferentes áreas e conduz mudanças.", exemplo: "Explica algo técnico para quem não é técnico — e as pessoas escutam quando essa pessoa fala.", icon: Megaphone },
  { title: "Responsabilidade e governança", desc: "Ética com dados, cuidado com riscos e clareza sobre limites da IA.", exemplo: "Pensa antes de colocar dados sensíveis da empresa numa ferramenta — e sabe que IA erra e precisa de revisão.", icon: ShieldCheck },
  { title: "Gestão de mudança", desc: "Conduz a empresa pela transição cultural exigida pela adoção da IA.", exemplo: "Quando a empresa muda um sistema ou processo, é quem puxa os outros em vez de resistir.", icon: Compass },
]
...
export function GuiaGuardiao({ onIr, slotAposPassos, slotAcoes }: { onIr: (tab: string) => void; slotAposPassos?: ReactNode; slotAcoes?: ReactNode }) {
  ...
  return (
    <div className="space-y-10">
      ...
      {/* 3 · O que o assessment mede — 5 pilares */}
      <section className="space-y-4">
        <div>
          ...
          {PILARES.map((p, i) => {
            ...
            return (
              <Card key={p.title} className={aberto ? "border-primary/30" : ""}>
                <button type="button" className="w-full text-left" onClick={() => setPilarAberto(aberto ? null : i)}>
